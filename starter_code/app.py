...
def search_venues():
  ...
  find_venues = Venue.query.filter(Venue.name.ilike("%{}%".format(request.form.get('search_term', '')))).all()
  response={
    "count": len(find_venues),
    "data": []
  }
...
def show_venue(venue_id):
  ...
  # no need for this filter because now i have only one data.
...
def search_artists():
  ...
  for artist in result_artists:
      response["data"].append({
          "id": artist.id,
          "name": artist.name,
          # "num_upcoming_shows": len(db.session.query(Show.start_time).filter(Show.artist_id == artist.id, Show.start_time >= datetime.now()).all())
      })
...
def create_artist_submission():
  ...
  try: 
    ...
    artist = Artist(name=name, city=city, state=state, phone=phone, image_link=image_link, facebook_link=facebook_link, genres = genres, website = website, seeking_venue = seeking_venue, seeking_description = seeking_description)
